...
func canMouseWin(grid []string, catJump int, mouseJump int) bool {
	...
	var bfs func(g map[pair][]pair, s, e pair) int
	bfs = func(g map[pair][]pair, s, e pair) int {
		q := []pair{s}
		vis := make([][]bool, m)
		for i := range vis {
			vis[i] = make([]bool, n)
	...
	for i := range grid {
		for j, v := range grid[i] {
			...
			for _, d := range dirs {
				if x, y := i+d.x, j+d.y; x >= 0 && x < m && y >= 0 && y < n && grid[x][y] != '#' {
					g[pair{i, j}] = append(g[pair{i, j}], pair{x, y})
				}
			}
		}
	}

	stepCat, stepMouse := bfs(g, cat, foot), bfs(g, mouse, foot)
	return stepCat <= stepMouse
}
...
func canMouseWin1(grid []string, catJump int, mouseJump int) bool {
